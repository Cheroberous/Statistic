...

{
    public partial class Form1 : Form
    {
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            g.Clear(pictureBox1.BackColor);
            draw_2_r();
            //Console.WriteLine("sto chiamando timer 2 a manetta");
            chart_c();
            chart_d();
            pictureBox1.Image = b;
        }

        private void draw_2_r()                            // QUI         gli istogrammi sono fatti da due funzioni separate (l'altra posso toglierla)
        {

            g.FillRectangle(Brushes.White, r5.r);
            g.DrawRectangle(Pens.Red, r5.r);
            g.FillRectangle(Brushes.White, r6.r);
            g.DrawRectangle(Pens.Red, r6.r);
        ...
        public void chart_c()
        {
            ...
            Point centro_sempre = new Point(r5.r.Left, ((r5.r.Bottom - r5.r.Top) / 2) + r5.r.Top);
            Point fine = new Point(r5.r.Right, ((r5.r.Bottom - r5.r.Top) / 2) + r5.r.Top);
            ...
            lunghezza_tratto_r5 = (r5.r.Right - r5.r.Left) / sample;
            altezza_tratto_r5 = (r5.r.Bottom - r5.r.Top) / sample;
            ...
            for (int j = 0; j < list_c.Count; j++)
            {
                ...
                PointF p0 = new PointF(r5.r.Left, ((r5.r.Bottom - r5.r.Top) / 2) + r5.r.Top);   // centro

                Pen new_p = pens[j];
                PointF next;

                for (int i = 0; i < list_c[j].Count; i++)                  //for (int i = 0; i < stat.Count; i++)
                {

                    float p1x = (float)p0.X + lunghezza_tratto_r5;
                    float p1y = (float)p0.Y + altezza_tratto_r5;
                    float p1y_m = (float)p0.Y - altezza_tratto_r5;
        ...
        public void insto_c(List<int> lis)
        {
            // fai ogni giorno unistogramma per ogni sistema (come hai fatto all'inizio)
            // introduci intervallo di tot secondi  dopo il nested for per dare tempo di plottare e guardare ??

            Rectangle r_insto;
            int index = 0;

            int x_s = (int)(r5.r.Left + (sample) * lunghezza_tratto_r5);     //  la x è questa

            int y_s = r5.r.Bottom;


            int y_f = r5.r.Top;
            int base_rett = (y_s - y_f) / linee;     // ok
            ...
            //quanto sono alti? --> quantivalori ci cadono (già in lista)

            for (int i = 0; i < lis.Count; i++)
            {
                // create rectangle i

                int lunghezza = (int)(lis[i] * (lunghezza_tratto_r5/2));
                r_insto = new Rectangle(x_s, y_s - (base_rett) * (index + 1), lunghezza, base_rett);
                //Console.WriteLine($"index == {index}");
                g.DrawRectangle(Pens.Black, r_insto);
                g.FillRectangle(Brushes.Red, r_insto);
                index++;
        ...
        public void chart_d()               // si alza tantissimo dati gli attacchi multipli
        {


         
            Point centro_sempre = new Point(r6.r.Left, ((r6.r.Bottom - r6.r.Top) / 2) + r6.r.Top);
            Point fine = new Point(r6.r.Right, ((r6.r.Bottom - r6.r.Top) / 2) + r6.r.Top);
            ...
            lunghezza_tratto_r6 = (r6.r.Right - r6.r.Left) / sample;
            altezza_tratto_r6 = (r6.r.Bottom - r6.r.Top) / sample;

            List<int> somme = new List<int>();



            for (int j = 0; j < list_d.Count; j++)
            {
                ...
                //PointF p0 = new PointF(r6.r.Left, ((r6.r.Bottom - r6.r.Top) / 2) + r6.r.Top);   // centro
                PointF p0 = new PointF(r6.r.Left, r6.r.Bottom );   // basso

                Pen new_p = pens[j];
                PointF next;

                for (int i = 0; i < list_d[j].Count; i++)             
                {

                    Console.WriteLine($"elaboro giorno {i} num attacchi == {list_d[j][i]}");
                    float p1x = (float)p0.X + lunghezza_tratto_r6;
                    float p1y = (float)p0.Y - (float)(altezza_tratto_r6 * (float)list_d[j][i]);
                    Console.WriteLine($"altezza punto == {p1y}");
                   


                    next = new PointF(p1x, p1y*0.1f);
        ...
        public void insto_d(List<int> lis)
        {
            // fai ogni giorno unistogramma per ogni sistema (come hai fatto all'inizio)
            // introduci intervallo di tot secondi  dopo il nested for per dare tempo di plottare e guardare ??
            ...
            int x_s = (int)(r6.r.Left + (sample) * lunghezza_tratto_r6);     //  la x è questa
            ...
            for (int i = 0; i < lis.Count; i++)
            {
                ...
                int lunghezza = (int)(lis[i] * (lunghezza_tratto_r6 / 45));
                r_insto = new Rectangle(x_s, y_s - (base_rett) * (index + 1), lunghezza, base_rett);
                //Console.WriteLine($"index == {index}");
                g.DrawRectangle(Pens.Black, r_insto);
                g.FillRectangle(Brushes.Red, r_insto);
        ...
        public void initialize_stat_c()
        {
            ...
            for (int i = 0; i < linee; i++)
            {
                ...
                for (int j = 0; j < sample; j++)         // quanti giorni
                {

                    randomProb = rand.NextDouble();                        // probabilità del giorno
                    randomFloat = rand.NextDouble();
                    //Console.WriteLine($"valore usci vs prob == {randomFloat} {prob}");
